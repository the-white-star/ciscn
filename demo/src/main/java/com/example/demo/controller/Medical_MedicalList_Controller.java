...
public class Medical_MedicalList_Controller {
    ...
    public @ResponseBody
    Map<String, Object> Medical_MedicalList_Mapper(@RequestParam("token") String token, HttpServletRequest request) {
            ...
            try {
                ...
                if (tmp == 0) {
                    ...
                    Log log = new Log((new SimpleDateFormat("yyyy-MM-dd HH:mm:ss")).format(new Date()),
                            "查看就诊信息", userID, IPUtils.getIPAddress(request), "数据模块");
    ...
    }//end Medical_MedicalList_Mapper
}
